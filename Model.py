...
def save(book):
    with open('telephone_directory.json', 'w', encoding="utf-8") as data:
...
def create_contact(book):
    ...
    contact = {"Last_name": last_name, "First_name": first_name,
               "Patronymic": patronymic, "Phone_numbers": num_tel}
    book.append(contact)
    book = sort(book)
    save(book)
    print('\nКонтакт создан')
...
def update_contact(book):
    ...
    if contact_change.isdigit() and int(contact_change) <= len(book):
        ...
        if num.isdigit()and (1 <= int(num) <= 4):
            ...
            if num == 4:
                book[contact_change-1]["Phone_numbers"] = input(f"Введите новый номер телефона пользователя {contact_change}: ")
        save(book)
...
def sort(book):
    ...
    for note in book:
        lst.append([note['Last_name'], note['First_name'],
                   note["Patronymic"], note["Phone_numbers"]])
    lst.sort()
    for note in lst:
        book_sort.append(
            {"Last_name": note[0], "First_name": note[1], "Patronymic": note[2], "Phone_numbers": note[3]})
    return book_sort
